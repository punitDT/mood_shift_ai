...
exports.buildStrongerSSML = buildStrongerSSML;
exports.buildCrystalSSML = buildCrystalSSML;
...
function removeProsodyArtifacts(text) {
...
function convertToXValue(value, attribute) {
    const rateMap = {
        "x-slow": "x-slow",
        "slow": "x-slow",
        "medium": "medium",
...
function convertToDecibels(volumeWord) {
    const volumeToDb = {
        "silent": "-20dB",
        "x-soft": "-10dB",
        "soft": "-6dB",
...
function buildSSML(text, engine, style, prosodyConfig) {
...
function buildStrongerSSML(text, engine) {
    const cleanedText = cleanTextForSpeech(text);
    const escapedText = escapeXml(cleanedText);
    if (engine === "generative") {
...
function buildCrystalSSML(text, engine) {
    const cleanedText = cleanTextForSpeech(text);
    const escapedText = escapeXml(cleanedText);
    if (engine === "generative") {
...
//# sourceMappingURL=ssmlService.js.map